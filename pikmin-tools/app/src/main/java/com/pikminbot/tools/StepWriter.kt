...
/**
 * Health Connect step writer for PikminBot Tools.
 *
 * APPENDS small records continuously while jogging: 2 steps/s accumulated and
 * flushed every 30 s. All records carry the `pikminbottools-` clientRecordId
 * prefix so they can be verified / deleted independently of the game's own
 * records (and of the old jogger app's `pikminjogger-` records).
 */
object StepWriter {
    private const val TAG = "PikminBotTools"
    const val PREFIX = "pikminbottools-"
    ...
    fun append(ctx: Context, start: Instant, end: Instant, steps: Long): String {
        return try {
            runBlocking {
                ...
                val device = Device(Device.TYPE_PHONE, "pikminbot", "tools")
