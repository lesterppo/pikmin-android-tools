 ...
 *   am start-foreground-service -n com.pikminbot.jogger/.JogService \
 *       --es mode loop --ef speed_kph 8.0 --ei heading 135 --ef steps_per_sec 2.5 \
 *       --ei radius_m 100 --ei duration_min 30 [--ef lat X --ef lon Y]
 *   am start-foreground-service -n com.pikminbot.jogger/.JogService --es cmd stop
 */
class JogService : Service() {
    ...
    companion object {
        ...
        fun speedMps(): Double = speedKph * 1000.0 / 3600.0
    }
    ...
    /** Same pattern as MockLoc: register the GPS test provider and push a fresh fix. */
    private fun inject() {
        try {
            try {
    ...
    private fun acquireWakeLock() {
        try {
            ...
            if (wl == null) {
                wl = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "pikminjogger:run")
                wl?.acquire(0)
            }
        } catch (t: Throwable) {
